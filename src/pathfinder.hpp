...
Inputs get_input_templated(const Board& b, const Move& target, const bool useFinesse, const bool force) {
    ...
    const int spawn = [&]{
        ...
        return (cm[Gen::SPAWN_COL][NORTH] & bb(R::SPAWN_Y)) ? 64 : R::SPAWN_Y;
    }();
    if (spawn == 64)
        return {};
    ...
    struct GhostMove {
        ...
        bool operator==(const GhostMove& other) const {
            return r == other.r && x == other.x && y == other.y && s == other.s;
        }

        static constexpr uint16_t root_index() {
